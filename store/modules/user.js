import { ACCESS_TOKEN, USER_ID,MY_TOKEN, MY_SELF } from '@/store/mutation-types'
...
const user = {
  ...
  actions: {
      ...
      return new Promise((resolve, reject) => {
        if (store.getters.userId > 0) {
          ...
          commit('SET_USER_ID', null)
					// 乐发行也清空
					storage.remove(MY_SELF)
					storage.remove(MY_TOKEN)
          resolve()
        }
      })
    },
	...
	register({commit},data){
		return new Promise((resolve, reject) => {
		  LoginApi.toRegister({ form: data }, { isPrompt: false })
		    .then(response => {
		      const result = response.data
		      loginSuccess(commit, result)
		      resolve(response)
